 ...
 * 敏感数据脱敏切面
 *
 * <p>拦截 Controller 返回的 {@link Result}，扫描 data 中所有标了 {@link SensitiveData} 的字段，
 * 递归处理嵌套对象和集合。</p>
 *
 * <p>权限豁免：</p>
 * <ul>
 *   <li>超级管理员（ADMIN）：默认可见明文（除非字段上 {@code maskForAdmin=true}）</li>
 *   <li>用户本人：可见自己的明文（基于 userId 字段匹配）</li>
 * </ul>
 *
 * <p>开关：{@code sensitive-data.enabled=false} 时全跳过</p>
 *
 * @author Mavis
 */
@Slf4j
...
public class SensitiveDataAspect {
    ...
    @Around("execution(* com.RD..controller..*(..))")
     ...
     * 递归脱敏对象
     *
     * <p>使用 {@code IdentityHashMap} 检测循环引用，防止 StackOverflow；
     * 递归向上遍历父类字段，匹配注解。</p>
     */
     ...
     *
     * <p>严格只匹配 {@code userId} 字段名，不匹配 {@code id}。
     * 原因：很多实体都有 id 字段（如 Task.id 是任务 ID），会误判。</p>
     */
     ...
     * 获取当前登录用户的 userId
     *
     * <p>统一用 {@link org.springframework.security.core.Authentication#getName()}，
     * 语义稳定且与 {@code JwtAuthenticationFilter} 存入 principal 的 userId 保持一致。</p>
     */
