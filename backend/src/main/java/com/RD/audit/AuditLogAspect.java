 ...
 * 审计日志切面
 *
 * 拦截标了 {@link AuditLog} 注解的方法：
 *   1) 执行前：收集入参快照、操作人、IP、UA
 *   2) 执行方法
 *   3) 执行后：收集返回快照
 *   4) 异步写入 {@code sys_audit_log} 表
 *
 * @author Mavis
 */
@Slf4j
...
public class AuditLogAspect {
    ...
    public Object around(ProceedingJoinPoint pjp, AuditLog auditLog) throws Throwable {
        ...
        // 只快照业务参数，过滤掉 HttpServletRequest 等框架对象
     ...
     * 解析资源 ID（支持 SpEL 表达式）
     *
     * 优先用注解指定的 SpEL 表达式；为空时找 @PathVariable 标注的数字参数；
     * 找不到就置 null（不强行猜）
     */
    ...
    /**
     * 过滤业务参数：只保留 @PathVariable / @RequestParam / @RequestBody 标注的参数
